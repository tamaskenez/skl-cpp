...
impurity_improvement(double impurity) const {
    ...
    ClassificationCriterion::ClassificationCriterion(NClassesRef n_classes)
    ...
    void ClassificationCriterion::
    init(sx::array_view<const DOUBLE_t, 2> y, sx::array_view<const DOUBLE_t> sample_weight,
    ...
    void Entropy::
    children_impurity(double* impurity_left,
                                double* impurity_right) const {
    ...
    inline RegressionCriterion::accumulators_t&
    RegressionCriterion::
    accu(SIZE_t idx) {
        return idx == 0
    ...
    inline const RegressionCriterion::accumulators_t&
    RegressionCriterion::
    accu(SIZE_t idx) const {
        return idx == 0
        ? first_output_accumulators
        : remaining_outputs_accumulators[idx-1];
    }
...
node_impurity() const {
    ...
    void BestSplitter::
    node_split(double impurity, SplitRecord* split,
...
build(Tree& tree, sx::matrix_view<const DTYPE_t> X, sx::matrix_view<const DOUBLE> y) {
    ...
    void DepthFirstTreeBuilder::
    build(Tree& tree, sx::matrix_view<const DTYPE_t> X,
        sx::matrix_view<const DOUBLE> y,
                             sx::array_view<const DOUBLE> sample_weight) {
        // check input
...
// Best first builder ----------------------------------------------------------
    ...
    void BestFirstTreeBuilder::
    _add_split_node(Splitter* splitter, Tree& tree,
